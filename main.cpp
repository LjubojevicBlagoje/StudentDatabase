...
void printGpaForStudent(Database& db, const std::string& studentId) {
    ...
    std::cout << "GPA for student " << studentId << " = " << gpa << " (0–7 scale)\n";
}
...
int main() {
    ...
    db.addStudent(std::make_unique<Student>("1", 6)); // 6 semesters
    db.addStudent(std::make_unique<Student>("2", 2)); // 2 semesters
    db.addStudent(std::make_unique<Student>("3", 1)); // 1 semester
    ...
    if (const Course* c = db.findCourseByCode("COMP1101")) {
        std::cout << "Found course: " << c->getCode()
                  << " - " << c->getName()
                  << " (" << c->getUnits() << " units)\n";
    } else {
    ...
    // Valid enrollments
    db.enrollStudentInCourse("1", "COMP1101", 2025, "S1", 6.0); // Distinction-ish
    ...
    printEnrollmentsForStudent(db, "3"); // removed student, but may still have none
