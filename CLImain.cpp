...
double readDouble(const std::string& prompt, double minVal, double maxVal) {
    while (true) {
        ...
        if (std::cin >> value) {
            ...
            std::cout << "Value must be in [" << minVal << ", " << maxVal << "]. Try again.\n";
        } else {
...
void actionAddStudent(Database& db) {
    ...
    bool ok = db.addStudent(std::make_unique<Student>(id, first, last, semesters));
...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1:  actionAddStudent(db); break;
            case 2:  actionRemoveStudent(db); break;
            case 3:  actionFindStudent(db); break;
            case 4:  actionAddCourse(db); break;
            case 5:  actionRemoveCourse(db); break;
            case 6:  actionFindCourse(db); break;
            case 7:  actionEnroll(db); break;
            case 8:  actionDrop(db); break;
            case 9:  actionUpdateGrade(db); break;
            case 10: actionShowEnrollments(db); break;
            case 11: actionComputeGpa(db); break;
            case 12: actionStudentsInCourse(db); break;
            case 13: actionCourseAverage(db); break;
            case 0:
                std::cout << "Goodbye!\n";
                return 0;
            default:
